...
def get_new_deploy_url() -> str | None:
    ...
    if "http_url" not in response_json or not response_json["http_url"]:
        print(f"'http_url' not found in response from {OFFSPOT_IMAGE_URL}")
...
def check_and_deploy():
    ...
    if not new_deploy_url:
        ...
        return
    print(f"Starting demo deployment with {new_deploy_url}")
    deploy_url(url=new_deploy_url, reuse_image=False)
    print("Deploy OK, persisting last image url")
    LAST_IMAGE_DEPLOYED_PATH.write_text(new_deploy_url)


def entrypoint():
    """Run continuously to periodically deploy new available image when updated"""
    check_and_deploy()
